/**
 * Cloudflare Worker API Routes for Linefulness
 * Handles CRUD operations for D1 database
 */
 ...
 */
async function handleHealth(request: Request, db: any): Promise<Response> {
  return jsonResponse({
    status: 'ok',
 ...
 */
async function handleNiches(request: Request, db: any, segments: string[]): Promise<Response> {
  const id = segments[0];
  
  switch (request.method) {
    case 'GET':
      if (id) {
        // Get single niche by slug or id
 ...
 */
async function handleLocations(request: Request, db: any, segments: string[], url: URL): Promise<Response> {
  const id = segments[0];
 ...
 */
async function handleTemplates(request: Request, db: any, segments: string[]): Promise<Response> {
  const id = segments[0];
  const url = new URL(request.url);
  const nicheId = url.searchParams.get('niche_id');
 ...
 */
async function handleConfig(request: Request, db: any, segments: string[]): Promise<Response> {
 ...
 */
async function handleNavigation(request: Request, db: any): Promise<Response> {
  if (request.method !== 'GET') {
    return jsonResponse({ error: 'Method not allowed' }, 405);
  }
